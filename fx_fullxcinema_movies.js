...
function parseArticles(html) {
  ...
  for (const c of chunks) {
    ...
    if (!aHref) continue;
    const title = decodeEntities(aHref[2]).trim();
...
async function main() {
  ...
  const listings = [
    [['latest', 1]],
    [['most-viewed', 2], ['most-viewed', 3], ['most-viewed', 4], ['most-viewed', 5], ['most-viewed', 6]],
    [['popular', 2], ['popular', 3]],
  ].flat();
  ...
  const quality = all.filter(it => {
    const hasYear = /\(\d{4}\)/.test(it.title);
    const slop = /\bporn(?:o| movie)?\b|\bever made\b|\bcollection\b|\bcompilation\b/i;
    return hasYear || !slop.test(it.title);
  });
  log(`Candidates after quality filter: ${quality.length}`);
  for (const it of quality.slice(0, 10)) log(`    ~ ${it.title}`);

  const chosen = [];
  for (const it of quality.slice(0, TOP_CANDIDATES)) {
    ...
    const film = {
      ...
      year: yearMatch ? parseInt(yearMatch[1], 10) : fmtDuration(it.duration),
      genreKey: 'adult',
      genreLabel: 'Adult',
      icon: '🔞',
      accent: '#b91c1c',
    ...
    log(`  ADDED: ${film.title} (${film.year}) ${v.status} ${v.bytes}B | ${mp4}`);
    await sleep(100);
  }
  ...
  if (DRY_RUN) {
    log('DRY RUN - skipping file patch and git');
    for (const f of chosen) log(`  [dry] ${f.title} (${f.year}) ${f.video}`);
    return;
  }
